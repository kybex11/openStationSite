    ...
    <div>
        ...
        <div class="game_board_wrap">
            ...
            <div class="mobile-controls">
              ...
              <button class="control-button" id="rotate-button">Rotate</button>
  ...
  for (let r = 0; r < shape.length; r++) {
    for (let c = 0; c < shape[r].length; c++) {
      if (shape[r][c]) {
        ...
        const sjfdf = document.getElementById("game_board") as HTMLDivElement
        sjfdf.appendChild(block);
      }
    }
  }
}
...
function canTetrominoMove(rowOffset: number, colOffset: number) {
  for (let i = 0; i < currentTetromino.shape.length; i++) {
    for (let j = 0; j < currentTetromino.shape[i].length; j++) {
      if (currentTetromino.shape[i][j] !== 0) {
...
function clearRows() {
  ...
  for (let y = BOARD_HEIGHT - 1; y >= 0; y--) {
    ...
    if (rowFilled) {
      ...
      const sdfjsdf = document.getElementById("game_board") as HTMLDivElement;
      sdfjsdf.innerHTML = "";
      for (let row = 0; row < BOARD_HEIGHT; row++) {
        for (let col = 0; col < BOARD_WIDTH; col++) {
          if (board[row][col]) {
            const block = document.createElement("div");
            block.classList.add("block");
            block.style.backgroundColor = board[row][col].toString();
            block.style.top = (row * 24).toString() + "px";
            block.style.left = (col * 24).toString() + "px";
            block.setAttribute("id", `block-${row}-${col}`);
            const sdjfsdfsdfsdf = document.getElementById("game_board") as HTMLDivElement;
            sdjfsdfsdfsdf.appendChild(block);
          }
        }
      }
...
// Move the tetromino
function moveTetromino(direction: any) {
  ...
  for (let r = 0; r < shape.length; r++) {
    for (let c = 0; c < shape[r].length; c++) {
      if (shape[r][c]) {
        ...
        const nisadfasdf = document.getElementById("game_board") as HTMLDivElement;
        nisadfasdf.appendChild(block);
...
function eraseGhostTetromino() {
  ...
  for (let ghostElement of Array.from(ghost)) {
    ghostElement.remove();
  }
}

// Check if tetromino can move in the specified direction
function canGhostTetrominoMove(rowOffset: any, colOffset: any) {
